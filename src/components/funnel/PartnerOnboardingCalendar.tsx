...
export default function PartnerOnboardingCalendar() {
    ...
    >
      <div className="p-6 border-b border-white/10">
        <h3 className="text-lg font-bold text-white font-[Geist,sans-serif]">
          Book Your Partner Onboarding Call
        </h3>
        <p className="text-blue-200/50 text-sm mt-1">Get set up for success in 30 minutes</p>
      </div>
          ...
          >
            {loading ? (
              ...
              <div className="p-6 text-center">
                ...
                <Button variant="outline" size="sm" className="border-white/10 text-white/70 hover:bg-white/5" onClick={() => fetchSlots(month)}>Retry</Button>
              </div>
            ) : (
                ...
                <div className="lg:border-r lg:border-white/10 lg:pr-4">
                  <Calendar
                    ...
                    classNames={{
                      ...
                      nav_button: "h-7 w-7 bg-white/5 border border-white/10 rounded-md p-0 text-white/60 hover:text-white hover:bg-white/10 inline-flex items-center justify-center",
                      ...
                      day_selected: "bg-cyan-500/20 text-cyan-300 border border-cyan-400/40 hover:bg-cyan-500/30 hover:text-cyan-200",
        ...
        {step === "details" && (
          ...
          >
            ...
            >
              ...
              {selectedDate && format(selectedDate, "MMM d")} at {selectedSlot && formatSlotTime(selectedSlot)}
            </button>

            <div className="space-y-3">
              <div>
                <Label htmlFor="partner-name" className="text-white/70 text-sm">Full Name *</Label>
                <Input id="partner-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Smith" className="mt-1 bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-cyan-400/40" maxLength={100} />
              </div>
              <div>
                <Label htmlFor="partner-email" className="text-white/70 text-sm">Email *</Label>
                <Input id="partner-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@example.com" className="mt-1 bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-cyan-400/40" maxLength={255} />
              </div>
              <div>
                <Label htmlFor="partner-phone" className="text-white/70 text-sm">Phone</Label>
                <Input id="partner-phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="(555) 123-4567" className="mt-1 bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-cyan-400/40" maxLength={20} />
              </div>
              <div>
                <Label htmlFor="partner-notes" className="text-white/70 text-sm">Questions for the call?</Label>
                <Textarea id="partner-notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Any questions about commissions, marketing materials, etc." className="mt-1 bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-cyan-400/40" rows={3} maxLength={500} />
              </div>
            </div>
        ...
        {step === "confirmed" && (
          ...
          >
            ...
            <p className="text-blue-200/60 text-sm max-w-xs mx-auto">
              {selectedDate && format(selectedDate, "EEEE, MMMM d, yyyy")} at {selectedSlot && formatSlotTime(selectedSlot)}
            </p>
            <p className="text-white/30 text-xs">
              Check your email for confirmation details. We'll walk you through everything on the call!
            </p>
          </motion.div>
        )}
      </AnimatePresence>
